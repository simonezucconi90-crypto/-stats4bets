...
def similarity(left, right):
    ...
    # Se un nome è contenuto nell'altro, è spesso la stessa squadra
    # con città/suffisso aggiuntivo (es. Kairat / Kairat Almaty).
...
def choose_fixture(db_match, fixtures):
    ...
    for fixture in fixtures:
        total, home_score, away_score = candidate_score(db_match, fixture)
        candidates.append((total, home_score, away_score, fixture))
...
def main():
    ...
    open_matches = (
        ...
        .select("id,date,time,match_name,pick,current_odds,outcome")
        .is_("outcome", "null")
        .order("date")
        .execute()
        .data
    ...
    for match in open_matches:
        ...
        if status not in FINISHED or home_goals is None or away_goals is None:
            waiting += 1
            continue
        ...
        values = {
            ...
            "profit": profit,
            "stake": STAKE,
            # Manteniamo played_odds allineata alla quota congelata,
            # ma il calcolo usa esplicitamente current_odds.
            "played_odds": float(match.get("current_odds") or 0),
        }
    ...
    print(
        ...
        f"{waiting} non terminate, {uncertain} non abbinate con sicurezza."
    )
